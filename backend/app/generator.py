...
2. КОНКРЕТИКА ОБЯЗАТЕЛЬНА. В каждой гипотезе — минимум один управляемый параметр с числом или направлением изменения:
   диаметр песковой насадки (мм), тонина помола (% класса -71 мкм), время флотации (мин), расход собирателя (г/т),
   плотность пульпы (% твёрдого), размер ячейки сита (мм). Числа бери из источников и диагноза; если источник даёт
   диапазон — укажи диапазон. ЗАПРЕЩЕНЫ формулировки «оптимизировать X» / «улучшить Y» без указания, что именно
...
5. Стиль формулировок — как у экспертов в примерах: короткое инженерное предложение + механизм + параметры.
   Типовые инженерные решения из примеров (замена насадок гидроциклонов, тонкое грохочение, магнитная сепарация,
   контрольная классификация, футеровка) применяй свободно, если они подходят под диагноз. НЕ переноси только
   привязки к чужим фабрикам: номера и размеры конкретных мельниц, марки реагентов, которых нет в источниках текущего объекта.
...
def generate_hypotheses(
    ...
    for i in range(n_samples):
        if i > 0:
            import time
            time.sleep(5)  # пауза между сэмплами под RPS-лимит YandexGPT
        try:
