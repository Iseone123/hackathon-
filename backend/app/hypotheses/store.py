...
from app.security.encryption import read_secure_json, write_secure_json
...
def load_generation(generation_id: str) -> dict | None:
    ...
    if not path.exists():
        return None
    return read_secure_json(path.read_text(encoding="utf-8"))


def load_hypothesis(hypothesis_id: str) -> Hypothesis | None:
    for path in settings.hypotheses_dir.glob("*.json"):
        data = read_secure_json(path.read_text(encoding="utf-8"))
